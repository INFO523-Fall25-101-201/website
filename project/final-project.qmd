...
The peer reviews will be completed during the peer-review week as indicated above. On those days teams will have access to the project repos of the teams whose work they're reviewing. The first peer review will be of proposals and the second peer review will be of code and organization.
...
# Read the CSV
p1_pr <- read_csv("final-project-peer-review.csv")

# Clean invisible/invalid characters in the link column
p1_pr <- p1_pr %>%
  mutate(link = str_replace_all(link, "[^[:ascii:]]", "")) # remove non-ASCII characters

# Create gt table with embedded link in the reviewer column
p1_pr %>%
  gt() %>% 
    cols_label(
      reviewer = "Reviewee",
      reviewee_1 = "Reviewer 1",
      reviewee_2 = "Reviewer 2",
      link = "Project Link"
  ) %>% 
  text_transform(
    locations = cells_body(columns = link),
    fn = function(x) {
      map_chr(x, ~ if (!is.na(.x) && .x != "") {
        as.character(html(glue("<a href='https://github.com/{.x}' target='_blank'>View Repo</a>")))
      } else {
        ""
      })
    }
...
Each of your projects will have a website. You can use the same workflow as for your first project or create something different. For example, if your project is building a dashboard, you might consider making your write-up a tab on that dashboard. Or if it's building a package, you might consider making your website using the [pkgdown](https://pkgdown.r-lib.org/) package. Feel free to Google your way around it or ask on the Slack channel and/or office hours!
